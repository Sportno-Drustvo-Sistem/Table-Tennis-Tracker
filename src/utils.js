...
export const getEloRank = (elo, isChampion = false) => {
    ...
    if (elo >= 1500) return { label: 'Diamond', color: '#8B5CF6', bg: 'bg-purple-100 dark:bg-purple-900/30' }
    if (elo >= 1325) return { label: 'Platinum', color: '#06B6D4', bg: 'bg-cyan-100 dark:bg-cyan-900/30' }
    if (elo >= 1150) return { label: 'Gold', color: '#F59E0B', bg: 'bg-yellow-100 dark:bg-yellow-900/30' }
    if (elo >= 950) return { label: 'Silver', color: '#9CA3AF', bg: 'bg-gray-200 dark:bg-gray-600/30' }
    if (elo >= 850) return { label: 'Bronze', color: '#cd7f32', bg: 'bg-amber-100 dark:bg-amber-900/30' }
