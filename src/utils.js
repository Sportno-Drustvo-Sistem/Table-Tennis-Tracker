...
export const getEloRank = (elo, isChampion = false) => {
    ...
    if (elo >= 1800) return { label: 'Master', color: '#EF4444', bg: 'bg-red-100 dark:bg-red-900/30' }
    if (elo >= 1600) return { label: 'Diamond', color: '#8B5CF6', bg: 'bg-purple-100 dark:bg-purple-900/30' }
    if (elo >= 1400) return { label: 'Platinum', color: '#06B6D4', bg: 'bg-cyan-100 dark:bg-cyan-900/30' }
    if (elo >= 1200) return { label: 'Gold', color: '#F59E0B', bg: 'bg-yellow-100 dark:bg-yellow-900/30' }
    if (elo >= 1000) return { label: 'Silver', color: '#9CA3AF', bg: 'bg-gray-200 dark:bg-gray-600/30' }
    if (elo >= 900) return { label: 'Bronze', color: '#cd7f32', bg: 'bg-amber-100 dark:bg-amber-900/30' }
